...
- 可选的“手动重新授权成功后自动同步”。默认关闭；开启后，单个或批量执行“重新授权”和“重新登录并授权”并成功完成时，会立即同步最新 OAuth 授权信息。若 CPAMP 明确显示该账号此前因失败已停用，AutoTo 会请求 CPAMP 重新校验新凭证并恢复启用；普通手动同步和无失败记录的停用账号仍保留原有启停状态。它不改变“自动同步以后新完成的任务”的规则，也不会作用于 Sub2API 异常巡检触发的自动重新登录。
