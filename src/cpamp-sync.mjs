...
export function createCpampSync(options) {
  ...
  async function syncJobs(jobs, syncOptions) {
    ...
    const context = {
      ...
      async getReauthorizationCandidates() {
        if (!this.reauthorizationCandidatesPromise) this.reauthorizationCandidatesPromise = listPendingReauthorizationCandidates();
        return this.reauthorizationCandidatesPromise;
      },
    };
  ...
  async function listPendingReauthorizationCandidates() {
    try {
      const payload = await request(config.baseUrl, managementKey, "/account-action-candidates?status=pending&limit=500");
      return extractActionCandidates(payload).filter((candidate) => candidate.actionType === "reauth");
    } catch (error) {
  ...
  async function resolveReauthorizationCandidates(candidates, email, file, fileName) {
  ...
  async function request(baseUrl, key, endpoint, options = {}) {
    const response = await requestRaw(baseUrl, key, endpoint, options);
  ...
  async function requestRaw(baseUrl, key, endpoint, options = {}) {
...
function selectRecoveredRemoteFile(files, file, fileName) {
  const target = remoteMutationTarget(file, fileName);
  return files.find((entry) => entry.name === fileName
    || remoteMutationTarget(entry, entry.name) === target) || null;
}

function isDisabledRemoteStatus(file) {
...
function buildAuthFileStatusPayload(file, targetName, disabled) {
  const physicalName = String(file?.name || "").trim();
  const runtimeId = String(file?.runtime_id ?? file?.runtimeId ?? "").trim();
  const authIndex = String(file?.auth_index ?? file?.authIndex ?? "").trim();
  const provider = String(file?.provider ?? "").trim();
  const accountId = String(file?.account_id ?? file?.accountId ?? "").trim();
  const accountSnapshot = String(file?.account_snapshot ?? file?.accountSnapshot ?? "").trim();
  return removeEmpty({
    name: targetName,
    disabled,
    auth_index: authIndex,
    cpamp_physical_name: physicalName,
    cpamp_runtime_id: runtimeId,
    cpamp_provider: provider,
    cpamp_account_id: accountId,
    cpamp_account_snapshot: accountSnapshot,
  });
}
  ...
  const values = Array.isArray(payload?.items)
    ...
    : Array.isArray(payload?.data?.items)
      ...
      : Array.isArray(payload)
        ...
        : [];
  return values
    .filter((candidate) => candidate && typeof candidate === "object" && String(candidate.id || "").trim())
    .map((candidate) => ({
      id: String(candidate.id).trim(),
      actionType: String(candidate.actionType ?? candidate.action_type ?? "").trim().toLowerCase(),
      status: String(candidate.status || "").trim().toLowerCase(),
      reason: remoteIssueDetail(candidate),
      reasonCode: String(candidate.reasonCode ?? candidate.reason_code ?? "").trim(),
      authFileName: String(candidate.authFileName ?? candidate.auth_file_name ?? candidate.fileName ?? candidate.file_name ?? "").trim(),
      authLabel: String(candidate.authLabel ?? candidate.auth_label ?? "").trim(),
      accountSnapshot: String(candidate.accountSnapshot ?? candidate.account_snapshot ?? "").trim(),
      runtimeId: String(candidate.runtimeId ?? candidate.runtime_id ?? "").trim(),
    }));
}

function actionCandidateEmail(candidate) {
  const value = [candidate?.accountSnapshot, candidate?.authLabel, candidate?.authFileName]
    .map((item) => String(item || "").match(/[a-z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-z0-9.-]+\.[a-z]{2,}/i)?.[0])
    .find(Boolean);
  const email = normalizeEmail(value);
  return isEmail(email) ? email : null;
}

function actionCandidateMatches(candidate, email, fileName, target) {
  const candidateEmail = actionCandidateEmail(candidate);
  const normalizedEmail = normalizeEmail(email);
  if (candidate?.authFileName && String(candidate.authFileName).trim() === String(fileName || "").trim()) return true;
  if (candidate?.runtimeId && String(candidate.runtimeId).trim() === String(target || "").trim()) return true;
